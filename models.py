...
class DynamicReasoningModel(LSTMReasoningModel):
    """Model with staged learning: grammar, semantics and cause-effect."""
    ...
    def _extract_definitions(self, line):
        # very naive extraction of "word = meaning" lines
        if '=' in line:
            word, meaning = line.split('=', 1)
            self.word_meanings[word.strip()] = meaning.strip()

    def _extract_cause_effect(self, line):
        if '->' in line:
            cause, effect = line.split('->', 1)
            self.knowledge_base.append(f"{cause.strip()} -> {effect.strip()}")
        elif 'because' in line.lower():
            parts = line.split('because')
            if len(parts) >= 2:
                effect = parts[0].strip()
                cause = parts[1].strip()
                self.knowledge_base.append(f"{cause} -> {effect}")

    def train_dynamic(self, file_path, epochs=3, seq_length=40, num_batches=100):
        if not os.path.exists(file_path):
            print(f"Eroare: Fișierul {file_path} nu există!")
            return
        # Stage 1: grammar and vocabulary
        super().train(file_path, epochs=epochs, seq_length=seq_length, num_batches=num_batches)
        # Stage 2 & 3: parse file once for definitions and cause-effect
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                self._extract_definitions(line)
                self._extract_cause_effect(line)

    def ask_meaning(self, word):
        return self.word_meanings.get(word, "Nu știu încă sensul acestui cuvânt.")
